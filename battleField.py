...
def draw_png(name, font_file, font_size = 36):
    ...
    _, imageArray = cv2.threshold(imageArray, 250, 255, cv2.THRESH_BINARY_INV | cv2.THRESH_OTSU)
    return imageArray
...
def extractNumber(img, templates, showImg=False):
    ...
    for x, y, w, h in result:
        ...
        predictVal = min(res, key=lambda x:x[1])[0]
...
class BattleField:
    ...
    def nGetDeckLeftCard(self, direc=Direction.SELF):
        ...
        if direc == Direction.SELF:
            icon = CoordinateIcon([(355, 460), (415, 556)])
